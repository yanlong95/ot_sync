...
def test_with_topN(topn, prediction_file_name, ground_truth_file_name):
...
if __name__ == "__main__":
    ...
    prediction_file_name = "/home/vectr/Documents/Dataset/test/test_results_botanical_garden/"
    # ============================================================================
    
    topn = 10  # for KITTI 00 top1%
    num_iter = 100
    # recall_list = []
    recall_list = np.zeros((num_iter-1, topn))

    for iter in tqdm.tqdm(range(1, num_iter)):
        prediction_file = os.path.join(prediction_file_name, f'predicted_des_L2_dis_20_60_train_iter_{iter}.npz')
        for i in range(1, topn + 1):
            # print("top"+str(i)+": ")
            rec = test_with_topN(i, prediction_file, ground_truth_file_name)
            # recall_list.append(rec)
            recall_list[iter-1, i-1] = rec
    # print(recall_list)
    # np.save("./test_results_kitti/recall_list", np.array(recall_list))

    for i in range(topn):
        x = np.arange(1, num_iter)
        y = recall_list[:, i]
        plt.plot(x, y, label=f'top {i}')

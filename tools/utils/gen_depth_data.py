...
if __name__ == '__main__':
    ...
    scan_folder = '/home/vectr/Documents/kitti_dataset_bin/data_odometry_velodyne/dataset/sequences'
    dst_folder = '/home/vectr/Documents/kitti_dataset_bin/data_odometry_velodyne/dataset/sequences'

    for i in range(22):
        if i < 10:
            input = f"{scan_folder}/0{i}/velodyne"
            output = f"{dst_folder}/0{i}"
        else:
            input = f"{scan_folder}/{i}/velodyne"
            output = f"{dst_folder}/{i}"

        depth_data = gen_depth_data(input, output)
